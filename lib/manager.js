(function() {
  var BBObject, Crypto, DeviceModel, Group, GroupModel, Linda, LindaSocketIO, Manager, TupleSpace, User, UserModel, mongoose, _,
  ...
  Manager = (function() {
    ...
    Manager.prototype.attach = function(io, server, app) {
      this.io = io;
      this.server = server;
      this.app = app;
      ...
      this.app.post("/api/user/new", this.User._create);
      this.app.get("/api/user/:name", this.User._read);
      this.app.put("/api/user/:name", this.User._update);
      this.app["delete"]("/api/user/:name", this.User._delete);
      this.app.post("/api/group/new", this.Group._create);
      this.app.get("/api/group/:name", this.Group._read);
      this.app.put("/api/group/:name", this.Group._update);
      return this.app["delete"]("/api/group/:name", this.Group._delete);
    };

    Manager.prototype.createUser = function(attrs, callback) {
    ...
    Manager.prototype.getUser = function(username, callback) {
      return User.find(username, callback);
    };

    Manager.prototype.createGroup = function(attrs, callback) {
      var owner;
      owner = attrs.owner;
    ...
    Manager.prototype.getGroup = function(attrs, callback) {
      return Group.find(attrs, callback);
    };

    Manager.prototype.Socket = {
      write: function(data) {},
      take: function(data) {},
    ...
    Manager.prototype.Session = {
      ...
      login: function(username, password, callback) {
        return User.find(username, password, function(result) {});
      }
    };
    ...
    Manager.prototype.User = {
      _create: function(req, res) {
        ...
        return Manager.create(attrs, res.json);
      },
      create: function(attrs, callback) {
        return User.create(attrs.name, attrs.password, function(user) {
          var data;
          data = user != null ? {
            status: false
          } : {
      ...
      _read: function(req, res) {
        console.log(Manager.User);
        return Manager.User.read(req.params.name, res.json);
      },
      read: function(name, callback) {
        return User.find(name, function(user) {
      ...
      _update: function(req, res) {
        ...
        return Manager.update(name, data, res.json);
      },
      update: function(attr, callback) {},
      _delete: function(req, res) {
        return Manager["delete"](req.aprams.name, res.json);
      },
      "delete": function(attr, callback) {
        return User.find(attrs.name, function(user) {
          var status;
          status = false;
          if (user) {
            user.remove();
    ...
    Manager.prototype.Group = {
      ...
      create: function(attrs, callback) {
        return Group.create(attrs.name, attrs.owner, function(result) {});
      },
      _read: function(req, res) {
        var name;
        name = req.params.name;
        return Manager.read(name, res.json);
      },
      read: function(name, callback) {
        return Group.find(name, function(result) {
          return callback(result);
        });
      },
      _update: function(req, res) {},
      update: function(name, attr) {},
      _delete: function(req, res) {},
  ...
  User = (function(_super) {
    ...
    User.create = function(username, password, callback) {
      ...
      return UserModel.findOne({
        ...
        if (user) {
          ...
          return callbacl.call(user, error, user);
        } else {
          u = new User();
    ...
    User.prototype["delete"] = function(username, password, callback) {
      var p;
      if (!this.isAuthenticate) {
        return callback(false);
      }
