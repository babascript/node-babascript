(function() {
  var EventEmitter, Linda, LindaBase, LindaClient, Person, Persons, TupleSpace, crypto, mm, moment, sync, sys, _,
  ...
  Person = (function(_super) {
    ...
    Person.prototype.humanExec = function(key, args) {
      var callback, cid, count, k, options, order, t, timeFormat, tuple, v, worker, _ref, _results,
        _this = this;
      cid = callbackId();
      options = {
        "cid": cid
      ...
      if (typeof args[args.length - 1] !== "function") {
        t = false;
        this.once("" + cid + "_callback", function(result) {
          t = true;
        ...
        while (!t) {
          console.log("hoge");
          _results.push(sync.await(setTimeout(sync.defer(), 1000)));
        }
        return _results;
      } else {
    ...
    Person.prototype._humanExec = function(key, args) {
      var done;
      if (typeof arg[args.length - 1] !== 'function') {
        return done = false;
      }
    };
