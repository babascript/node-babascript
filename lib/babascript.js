(function() {
  var Baba, Human, Linda, LindaBase, LindaClient, People, TupleSpace, crypto, mm, moment, sys,
  ...
  Human = (function() {
    ...
    function Human(id, linda) {
      var _this = this;
      this.ts = new TupleSpace(id, linda);
      this.tasks = [];
    ...
    Human.prototype.methodmissing = function(key, args) {
      if (key === "inspect") {
  ...
  People = (function() {
    ...
    function People(name, linda) {
      this.humanExec = __bind(this.humanExec, this);
      this.methodmissing = __bind(this.methodmissing, this);
      var _this = this;
      this.ts = new TupleSpace(name, LindaBase);
      this.tasks = [];
      this.members = [];
      this.connecting = false;
      if (LindaBase == null) {
        LindaBase = new Linda();
        LindaBase.io.on("connect", function() {
    ...
    People.prototype.methodmissing = function(key, args) {
      if (key === "inspect") {
        return sys.inspect(this);
      }
      if (this.connecting) {
    ...
    People.prototype.humanExec = function(key, args) {
      var cid, count, hash, options, order, t, timeoutFlag, tuple,
        _this = this;
      ...
      return this.ts.take(["babascript", "return", cid], function(_tuple, info, list) {
        var r, result, temp, _i, _len, _ref;
        timeoutFlag = false;
        if (count > 0) {
          count--;
